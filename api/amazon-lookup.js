...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Estrai il primo ASIN dalla pagina di ricerca
    const asinMatch = searchHtml.match(/data-asin="([A-Z0-9]{10})"/)
    if (!asinMatch) return res.status(404).json({ error: 'not_found' })
